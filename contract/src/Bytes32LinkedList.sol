...
library Bytes32LinkedList {
    bytes32 internal constant SENTINEL_BYTES32 =
    ...
    function add(
        ...
        bytes32 data
    ) internal onlyBytes32(data) {
        if (self[data] != bytes32(0)) {
            revert("data already exists");
        }
    ...
    function isExist(
        mapping(bytes32 => bytes32) storage self,
        bytes32 data
    ) internal view onlyBytes32(data) returns (bool) {
